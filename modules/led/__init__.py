...
def command_led_set(arg, argc):
	if 0 < argc < 3:
		if not client.configuration.get("led_server"): return messagetypes.Reply("No LED server set (with 'cfg led_server')")
		...
		try:
			res = requests.post(client.configuration["led_server"], data={"pos":value, "color":color}).json()
