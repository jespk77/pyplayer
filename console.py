...
class TextConsole(tkinter.Text):
	...
	def __init__(self, root, configuration=None, command_callback=print):
		...
		if configuration != None: self.set_configuration(configuration)
	...
	def set_configuration(self, configuration):
		if isinstance(configuration, dict):			
			for tag, value in configuration.items():
				...
				if len(tag) > 1:
					try:
						if tag[0] == "font": self.font[tag[1]] = values
	...
	def clear_current_line(self, event=None):
		if event != None: self.last_line = None
	...
	def on_command_confirm(self, event):
		...
		if len(cmd) > 0:
			...
			if self.cmd_callback != None: self.cmd_callback(cmd)
		return self.block_action(event)
	...
	def on_set_command_from_history(self, event):
		...
		if last_index >= 0:
			self.mark_set("insert", "end")
			if event.keysym == "Up":
				if self.last_line == None: self.last_line = self.get(self.current_start, self.current_end)
	...
	def set_reply(self, msg=None, tags=()):
		if self.cget("state") == "disabled":
			...
			if msg != None: self.insert("end", msg + "\n", tags)
